// class include algorithms 1 and 2 as described in Matala2.
// For testing , please change line 22 for different output file path
...
public class Algorithem {
	...
	// ****FOR testing in different OS , Change path of UOTPUT path of the bellow******
	//OUTPUT PATH:
	...
	public Algorithem()
	{
		...
		MakeKML test=new MakeKML(halfkmloutputpath + "outputCSV.csv");
		this.Data=test.inputthefile();
	}
	public Algorithem(List<ArrayList<String>> Data)
	{
		this.Data=Data;
	}
	...
	//mylist = list of macs identical to the input mac.
		...
		List<Signal_index> signals=new ArrayList<Signal_index>();
			...
			for(int j=6; j<Data.get(i).size(); j+=4)
			{
				if(MAC.equals(Data.get(i).get(j)))
				{
					if(!Mylist.contains(Data.get(i)))
					{
						...
						Signal_index temp=new Signal_index(signal,Mylist.size()-1);
						signals.add(temp);
					}
				}
			}
		...
		// sort for s_i objects.
		Signal_indexbubbleSort(signals);
	...
	// calculate the wheight of the point.
	...
	public Point GetUserLocation(Mac_Signal[] arr) 
	{
	...
	public static void main(String[] args) throws FileNotFoundException {	
		...
		Algorithem test=new Algorithem();
		//test algo 1
	...
	public static void Signal_indexbubbleSort(List<Signal_index> arr)
	{  
		int n = arr.size(); 
		Signal_index temp;
		for(int i=0; i < n; i++)
